...
function installOllama() {
  return new Promise((resolve) => {
    exec('which ollama', (err) => {
      ...
      install.stdout.on('data', d => process.stdout.write(d));
      install.stderr.on('data', d => process.stderr.write(d));
      install.on('close', (code) => {
        if (code === 0) {
          console.log('Ollama installed successfully.');
        } else {
          console.error(`Ollama install exited with code ${code}.`);
        }
        resolve();
      });
    });
    ...
    ollamaGetJSON('/api/tags').then((data) => {
      ...
      pull.on('close', (code) => {
        if (code === 0) {
          ...
          console.error(`Failed to pull ${model} (exit ${code}).`);
        }
        resolve();
      });
    });
